...
public class JwtUtil {
    ...
    public String generateAccessToken(String email, UUID merchantId, String role){
        ...
        return Jwts.builder().subject(email).issuedAt(Date.from(now)).expiration(Date.from(now.plusSeconds(60*100)))
                .claim("merchant_id",merchantId).claim("role",role).signWith(getSecretKey()).compact();
    }
